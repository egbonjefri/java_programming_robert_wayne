...
public final class StdRandom {
    ...
    public static int uniformInt(int a, int b) {
        if ((b <= a) || ((long) b - a >= Long.MAX_VALUE)) {
    ...
    /*
     * The Marsaglia method is more efficient as it avoids 
     * calculating trigonometric functions, and it also generates
     * a spare normally distributed number for each pair of uniform random numbers, 
     * reducing the number of times the loop needs to be executed.
     */
    public static double gaussianMarsaglia() {
        if (hasSpare) {
            // Use the spare generated from the previous call
            hasSpare = false;
            return spare;
        } else {
          double r, x, y;
        do {
            ...
            // Marsaglia's polar method formula: 
            // z0 = sqrt(-2 * ln(r) / r) * x
            // z1 = sqrt(-2 * ln(r) / r) * y
            // z0 and z1 are independent standard normal variates
            double multiplier = Math.sqrt(-2.0 * Math.log(r) / r);
            spare = y * multiplier;
            hasSpare = true;
            return x * multiplier; // Returning z0; z1 is stored as 'spare' for the next call
        }
    }
    /**
     ...
     * the exponential distribution or negative exponential distribution
    ...
    /**
     * Returns a random number drawn from the Maxwell-Boltzmann distribution with parameter sigma.
     * @param sigma The standard deviation parameter for the Gaussian components.
     * @return A random number from the Maxwell-Boltzmann distribution.
     */
    public static double maxwellBoltzmann(double sigma) {
        ...
        // Generate three independent Gaussian-distributed random numbers
        double gaussian1 = gaussian() * sigma;
        double gaussian2 = gaussian() * sigma;
        double gaussian3 = gaussian() * sigma;

        // Return the square root of the sum of the squares of the three numbers
        return Math.sqrt(gaussian1 * gaussian1 + gaussian2 * gaussian2 + gaussian3 * gaussian3);
    }



    /**
     * Scales the values in array a[] to be between ymin and ymax.
     * @param ymin The minimum value after scaling.
     * @param ymax The maximum value after scaling.
     * @param a The array to scale.
     */
    public static double[] scaleToRange(double ymin, double ymax, double[] a) {
        double[] array1 = new double[a.length];
        if (ymin > ymax) throw new IllegalArgumentException("ymin must be less than ymax");
        ...
        if (min == max) {
            for (int i = 0; i < a.length; i++) {
                array1[i] = ymin;  // or (ymin + ymax) / 2 if you prefer the middle of the range
            }
        } else {
            // The linear transformation formula used is:
            // y = (x - min) * (ymax - ymin) / (max - min) + ymin
            // Where x is the original value and y is the scaled value.
            for (int i = 0; i < a.length; i++) {
                array1[i] = (a[i] - min) * (ymax - ymin) / (max - min) + ymin;
            }
        }
     ...
     * @param a The array to scale.
     */
    public static int[] scaleToRange(int ymin, int ymax, int[] a) {
    ...
    public static double[] scaleArrayToUnitInterval(double[] array) {
        double min = Double.MAX_VALUE;
        double max = Double.MIN_VALUE;
    ...
    public static double[] scaleArrayToUnitInterval(int[] array) {
        int min = Integer.MAX_VALUE;
        int max = Integer.MIN_VALUE;
