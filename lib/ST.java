...
public class ST<Key extends Comparable<Key>, Value> implements Iterable<Key> {
    ...
    public Value getOrDefault(Key key, Value defaultValue) {
        if (key == null) {
            throw new IllegalArgumentException("Called getOrDefault() with null key");
        }
        Value value = st.get(key);
        if (value != null) {
            return value;
        }
        st.put(key, defaultValue);
        return defaultValue;
    }
        /**
     * Inserts the specified key-value pair into the symbol table only if the key is not already present.
     * 
     * @param key the key
     * @param value the value
     * @return the current value associated with the key, or null if the key was absent and the new pair was inserted.
     */
